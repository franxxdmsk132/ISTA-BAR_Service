...
public class PedidoService {
  ...
  //Método para devolver los pedidos con su respectivo detalle por id
  ...
  public GenericResponse<List<PedidoConDetallesDTO>> devolverPedidoConDetalle() {
    final List<PedidoConDetallesDTO> dtos = new ArrayList<>();
    final Iterable<Pedido> pedidos = repository.findAll();
    pedidos.forEach(p -> {
      dtos.add(new PedidoConDetallesDTO(p, detallePedidoRepository.findByPedido(p.getId())));
    });
    return new GenericResponse(OPERACION_CORRECTA, RPTA_OK, "Petición Encontrada", dtos);
  ...
  public GenericResponse<List<PedidoCompletoDTO>> listarPedidosConDetalleYCliente() {
    List<PedidoCompletoDTO> listaPedidosConDetalleYCliente = new ArrayList<>();

    List<Pedido> pedidos = (List<Pedido>) this.repository.findAll();

    for (Pedido pedido : pedidos) {
      // Convierte el resultado a una lista usando el constructor del ArrayList
      List<DetallePedido> detalles = new ArrayList<DetallePedido>((Collection<? extends DetallePedido>) this.detallePedidoRepository.findByPedido(pedido.getId()));
      listaPedidosConDetalleYCliente.add(new PedidoCompletoDTO(pedido, pedido.getCliente(), detalles));
    }

    return new GenericResponse<>(OPERACION_CORRECTA, RPTA_OK, "Lista de pedidos con detalle y cliente", listaPedidosConDetalleYCliente);
  }
  ...
  public ResponseEntity<Resource> exportInvoice(int idCli, int idOrden) {
    ...
    if (optPedido.isPresent()) {
      try {
        ...
        final File imgLogo = ResourceUtils.getFile("classpath:images/logotec.png");
